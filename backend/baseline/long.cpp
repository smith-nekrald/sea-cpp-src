...
ByTotalProfit::ByTotalProfit(const AllotmentSorterConfig& config)
    : input(config.inputManager->getConstData())
    , links(config.linksManager->getConstData()) {}

double ByTotalProfit::getTotalProfit(unsigned allotmentId) const {
...
std::vector<unsigned> ByTotalProfit::selectOrder() {
    ...
    std::sort(std::begin(allotmentOrder), std::end(allotmentOrder),
            [&](unsigned lhs, unsigned rhs) {
                return getTotalProfit(lhs) > getTotalProfit(rhs);
            });
...
LongCompositeSorter::LongCompositeSorter(const AllotmentSorterConfig& config)
    ...
    sorters.push_back(std::make_unique<ByTotalProfit>(config));
}
...
std::vector<unsigned> LongCompositeSorter::selectOrder() {
    ...
    for (const auto& orderer: sorters) {
        auto candidateOrder = orderer->selectOrder();
